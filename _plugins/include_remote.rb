...
eos
  ...
  def render(context)
    ...
    if ! html = @@remote_cache["#{@url}_#{xpath}"]
      # fetch remote file
      # ssl_verify_mode does not work w/ ruby 2.2.1
      htmlFile = open(@url, {ssl_verify_mode: OpenSSL::SSL::VERIFY_NONE})
      #htmlFile = open(@url)
      page = Nokogiri::HTML::DocumentFragment.parse(htmlFile.read, 'utf-8')
