...
if (!isset($_SESSION['dicos'])) {
  $_SESSION['dicos'] = "LSJ,Pape,Bailly,B_Abr";
}
if (isset($_POST['dicos'])) {
  $_SESSION['dicos'] = $_POST['dicos'];
}
$dicos = explode(",", $_SESSION['dicos']);
...
if (!$consultation) {
  ...
  $le_LSJ = in_array("LSJ", $dicos);
  $le_Pape = in_array("Pape", $dicos);
  $le_Bailly = in_array("Bailly", $dicos);
  $le_B_Abr = in_array("B_Abr", $dicos);
  ...
  if ($pos_ind > 2000) {
    // Le 2 mai 2020 : j'étends la gamme de recherche de 1000 à 2000.
    fseek($f_index, $pos_ind - 2000);
  ...
  /*            if ($eclats[1] != '') {
   ...
   if ($eclats[5] != '') {
   $pos_B_Abr[$y_B_Abr] = $eclats[5];
   $len_B_Abr[$y_B_Abr] = strlen($eclats[6]);
   $y_B_Abr += 1;
   }
   */
  $avant = -1;
  $ici = 0;
  while ((ftell($f_index) < $pos_ind + 2000) && !feof($f_index)) {
    $a[$x] = ftell($f_index);
    ...
    if ($eclats[0] == $lemme) {
      ...
      if ($eclats[7] != '') {
        $pos_Bailly[$y_Bailly] = $eclats[7];
        $len_Bailly[$y_Bailly] = strlen($eclats[8]);
        $y_Bailly += 1;
// Ajout du Bailly 2020, le 2 mai 2020.
      }
    } else if ($eclats[0] < $lemme)
      $avant = $x;
    $x = $x + 1;
  ...
  $quelque_chose = ($y_LSJ != 0 && $le_LSJ) || ($y_Pape != 0 && $le_Pape) || ($y_B_Abr != 0 && $le_B_Abr) || ($y_Bailly != 0 && $le_Bailly);
...
//        $l_ii = liste_pos($pape, " <i>= ", $l_ii);
  ...
  if ($y_B_Abr != 0 && $le_B_Abr) {
//    $f1 = fopen("data/Bailly.Abr.5.1.txt", "r");
// Juin 2017 : le XMLBailly351 devient Bailly.Abr.4.3
    $f1 = fopen("data/" . $B_Abr_name, "r");
    for ($i = 0; $i < $y_B_Abr; $i++) {
      fseek($f1, $pos_B_Abr[$i]);
      $lsj = fgets($f1);
      $lsj = substr_replace($lsj, "</b></span>", $len_B_Abr[$i], 0);
      $lsj = "<span style='color:red'><b>" . $lsj;
      $ii = 0;
      // if (stripos($lsj, " c.") > 0)
    ...
    fclose($f1);
    $B_Abr_tot = str_replace("\\n<b>", "<br />\n<b>", $B_Abr_tot);
    // Un peu de mise en forme...
    $B_Abr_tot = str_replace("\\n  ", "<br />\n  ", $B_Abr_tot);
    $B_Abr_tot = str_replace("\\n", "\n", $B_Abr_tot);
  }
  if ($y_Bailly != 0 && $le_Bailly) {
// Le 2 mai 2020 : j'ajoute le Bailly
...
//      $lsj = "<span style='color:red'><b>" . $lsj;
// Le Bailly a déjà le lemme en rouge.
      $ii = 0;
      $l_ii = [];
      $l_ii[0] = 0;
      if (stripos($lsj, " c.") > 0) $l_ii = liste_pos($lsj, " c.", $l_ii);
